    ...
    <el-drawer v-model="drawer" size="720" :title="t('agentCandidates.candidateDetail')">
      <div v-if="selected" class="detail">
        ...
        <section v-if="diffLoading || diffSource != null || traceLoaded" class="diff-block">
          <div class="diff-head">
            <h4>{{ t('agentCandidates.diffTitle') }}</h4>
...
import { approveCandidate, getCandidateTrace, listAgentCandidates, rejectCandidate, type AgentCandidate } from '../api/agent'
...
async function loadTrace(id: string) {
  ...
  diffMode.value = 'line'
  try {
    const trace = await getCandidateTrace(id)
    const snapshot = trace.task.raw_text_snapshot?.trim() ? trace.task.raw_text_snapshot : null
    const raw = snapshot ?? (trace.report?.raw_text?.trim() ? trace.report.raw_text : null)
    if (raw != null) {
