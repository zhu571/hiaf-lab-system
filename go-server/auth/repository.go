	...
	err := r.db.QueryRow(
		`INSERT INTO users (username, password_hash)
		 ...
		 RETURNING id, username, password_hash, display_name, role, must_change_pw,
		           failed_attempts, token_version, locked_until, created_at, updated_at`,
		username, passwordHash,
	).Scan(
		&user.ID, &user.Username, &user.PasswordHash, &user.DisplayName, &user.Role,
		&user.MustChangePW, &user.FailedAttempts, &user.TokenVersion, &lockedUntil, &user.CreatedAt, &user.UpdatedAt,
	)
	if err != nil {
