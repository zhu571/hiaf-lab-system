...
type row struct {
	ID, EntityType, EntityID, FieldName, TargetLocale, SourceLocale, SourceHash, Text, Status, Origin, Model, PromptVersion, ErrorCode string
	Attempts                                                                                                                           int
	UpdatedAt                                                                                                                          time.Time
}
...
ON CONFLICT (entity_type,entity_id,field_name,target_locale) DO UPDATE SET source_locale=EXCLUDED.source_locale,source_hash=EXCLUDED.source_hash,status=CASE WHEN content_translations.origin='manual' AND NOT $8 THEN 'ready' ELSE 'pending' END,translated_text=CASE WHEN content_translations.origin='manual' AND NOT $8 THEN content_translations.translated_text ELSE NULL END,origin=CASE WHEN content_translations.origin='manual' AND NOT $8 THEN 'manual' ELSE 'ai' END,attempts=0,next_attempt_at=now(),locked_until=NULL,error_code=NULL,requested_by=EXCLUDED.requested_by,updated_at=now()
WHERE content_translations.source_hash <> EXCLUDED.source_hash OR $8`, entityType, entityID, field, target, sourceLocale, hash, requestedBy, force)
	return err
}
...
func (r *Repository) Claim(ctx context.Context) (*row, error) {
	var x row
	err := r.db.QueryRowContext(ctx, `WITH picked AS (SELECT id FROM content_translations WHERE (status='pending' OR (status='processing' AND locked_until < now())) AND next_attempt_at <= now() AND attempts < 3 ORDER BY next_attempt_at FOR UPDATE SKIP LOCKED LIMIT 1) UPDATE content_translations t SET status='processing',locked_until=now()+interval '2 minutes',attempts=attempts+1,updated_at=now() FROM picked WHERE t.id=picked.id RETURNING t.id,t.entity_type,t.entity_id,t.field_name,t.target_locale,t.source_locale,t.source_hash,COALESCE(t.translated_text,''),t.status,t.origin,COALESCE(t.model,''),COALESCE(t.prompt_version,''),COALESCE(t.error_code,''),t.attempts,t.updated_at`).Scan(&x.ID, &x.EntityType, &x.EntityID, &x.FieldName, &x.TargetLocale, &x.SourceLocale, &x.SourceHash, &x.Text, &x.Status, &x.Origin, &x.Model, &x.PromptVersion, &x.ErrorCode, &x.Attempts, &x.UpdatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	return &x, err
}
func (r *Repository) Complete(id, hash, text, model, prompt string) error {
	_, err := r.db.Exec(`UPDATE content_translations SET translated_text=$3,status='ready',model=$4,prompt_version=$5,locked_until=NULL,error_code=NULL,updated_at=now() WHERE id=$1 AND source_hash=$2 AND status='processing' AND origin='ai'`, id, hash, text, model, prompt)
	return err
}
func (r *Repository) Fail(id, code string, retry bool) error {
	status := "failed"
	if retry {
		status = "pending"
	}
	_, err := r.db.Exec(`UPDATE content_translations SET status=$2,error_code=$3,next_attempt_at=now()+CASE WHEN attempts=1 THEN interval '5 seconds' ELSE interval '30 seconds' END,locked_until=NULL,updated_at=now() WHERE id=$1 AND status='processing'`, id, status, code)
	return err
}
