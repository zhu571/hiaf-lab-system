...
var (
	...
	writeRe    = regexp.MustCompile(`(?i)\b(insert|update|delete|merge|copy)\b`)
...
func prepareSQL(raw string) (sqlText, table string, truncated bool, err error) {
	...
	if writeRe.MatchString(sqlText) {
		return "", "", false, fmt.Errorf("%w: 禁止写语句（INSERT/UPDATE/DELETE/MERGE/COPY）", ErrSQLRejected)
	}
	lower := strings.ToLower(sqlText)
	for _, banned := range []string{"pg_", "information_schema", "current_setting", "current_user"} {
		if strings.Contains(lower, banned) {
			return "", "", false, fmt.Errorf("%w: 禁止访问系统对象 %q", ErrSQLRejected, banned)
		}
	}
	if versionRe.MatchString(sqlText) {
	...
	if m := limitNumRe.FindStringSubmatchIndex(sqlText); m != nil {
		n, _ := strconv.Atoi(sqlText[m[2]:m[3]])
		if n > maxRows {
			sqlText = sqlText[:m[0]] + "LIMIT 200" + sqlText[m[1]:]
			truncated = true
		}
	} else if loc := limitRe.FindStringIndex(sqlText); loc != nil {
...
// 跨表引用都会产生多个表名（随后按"仅单张白名单主表"拒绝）。遇字符串字面量中的
// "from xxx" 会严格解析（可能误拒，但只会拒不会放行——纵深防线以安全优先）。
func extractFromTables(sqlText string) ([]string, error) {
	var tables []string
	for _, loc := range fromKwRe.FindAllStringIndex(sqlText, -1) {
		rest := sqlText[loc[1]:]
		for {
...
// normalizeValue 行集序列化规则：time→RFC3339、[]byte 跳过（JSONB 保留结构、
// 二进制置 null）、float/其他原样（方案 §5）。
