...
日志/日报翻译读取和按需入队先经过所属对象的既有权限；人工保存和 `force` 重生成要求对象更新权限。翻译表不读取业务表，写请求继续经过 CSRF、幂等键和 Audit；审计只记录对象、字段、语言、hash 和状态，不记录原文、译文或完整 prompt。py-agent 翻译端点仅接受内部 Bearer token，正文作为不可信输入且禁用工具。
